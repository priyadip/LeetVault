...
def write_analysis_md(
    ...
    p_dir = problem_dir(repo_path, problem.title_slug)
    p_dir.mkdir(parents=True, exist_ok=True)
    # Provider/model only, so a reader can tell which model produced the analysis - and
    # discount it accordingly, since a 7B local model and a frontier model are not the
    # same claim about the code.
