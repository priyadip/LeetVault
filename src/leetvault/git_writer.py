...
def analysis_md_path(repo_path: Path, title_slug: str) -> Path:
...
def write_analysis_md(
    ...
    """Write AI-generated analysis to its own file.

    Deliberately not notes.md: that file is the user's own scratch space and is never
    overwritten once created, so generated content must not land there.
    """
    p_dir = problem_dir(repo_path, problem.title_slug)
    p_dir.mkdir(parents=True, exist_ok=True)
    # Provider/model only. Third-party attribution for the prompt lives in the package's
    # NOTICE.md, which is what the MIT licence requires - generated files are output, not
    # copies of the software, so they carry no third-party notice.
    footer = f"_Generated by leetvault using {provider}" + (f" ({model})_" if model else "_")
    content = "\n".join(
        [
            f"# {problem.frontend_id}. {problem.title} - Solution Analysis",
            "",
            body.strip(),
            "",
            "---",
    ...
    path = analysis_md_path(repo_path, problem.title_slug)
